#!c:\Python27\python
####################################################################
#  Testscript: ts_ospf_func_intra_route_prefer_001.py
#  Purpose  :  To verify that DUT prefer intra area route over inter 
#               area route.
#
#  Detail   : The DUT when configured to have route to destination (R2) both
#             intra area (via R1) as well as inter area (via R2) , prefer
#             intra area (via R1) route to reach to destination
#
#                          A0
#Topology  :        DUT --------- R1
#                    \             /
...
interface_list = [R1.device_interface_1, R1.device_interface_2]
ip_address_list = ['10.1.0.2', '12.1.2.1']
...
if show_ip_route == "" :
    log_print("Step 7: No Routing information",1)
    exit
else :
    log_print("Step 7: Found Routing entry",1)
...
if route_type == "" :
                   log_print(" Step 7: Test Abort: No route entry for network %s" %dut_network,1)
                   exit

if not ospf_is_route_type_IA(route_type):
    log_print("Step 7: TEST PASSED: Found Intra route for network %s on DUT" %dut_network,1)
else:
    log_print("Step 7 TEST FAILED: Inter Area route found instead of intra area for network %s in DUT" %dut_network,1)
log_print( " ")
